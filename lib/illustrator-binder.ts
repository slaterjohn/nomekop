...
// Token URLs: /illustrator/<slug>~<rows><cols><order>
...
export function encodeIllustratorToken(name: string, options: IllustratorBinderOptions): string {
  return `${slugifyArtistName(name)}~${options.rows}${options.cols}${ORDER_CH[options.order]}`;
}

const TOKEN_RE = /^([a-z0-9.'\-:♀♂é ]{2,40})~([1-5])([1-5])([no])$/i;
