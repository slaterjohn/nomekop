...
describe("LegalPage", () => {
  ...
  it("states clearly that it is not affiliated, naming Nintendo and The Pokémon Company", () => {
    render(<LegalPage />);
  ...
  it("credits the Pokémon TCG API with a link to pokemontcg.io", () => {
    render(<LegalPage />);
  ...
  it("credits the PokeAPI sprite collection with a link to its GitHub repo", () => {
    render(<LegalPage />);
  ...
  it("reassures that collection data lives only in the browser's local storage", () => {
    render(<LegalPage />);
    expect(screen.getByText(/local storage/i)).toBeInTheDocument();
    expect(screen.getByText(/no accounts/i)).toBeInTheDocument();
  });

  it("offers a mailto contact link for takedowns", () => {
    render(<LegalPage />);
